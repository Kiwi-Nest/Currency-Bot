...
class Sell(commands.Cog):
    ...
    organs = ["Brain", "Heart", "Left Kidney", "Right Kidney", "Left Lung", "Right Lung", "Liver", "Bone Marrow"]
    ...
    @app_commands.choices(organ=[app_commands.Choice(name=organ, value=organ.lower()) for organ in organs])
    async def harvest(self, ctx: commands.Context, organ: str = None):
        await ctx.defer()
    ...
    async def sell_error(self, ctx: commands.Context, error):
        if isinstance(error, commands.CommandOnCooldown):
            ...
            await ctx.send(f"Please wait {minutes}m {seconds}s before repeating this command.")
